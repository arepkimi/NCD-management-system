<?php
  ...
  if(isset($_POST['submit'])){
    ...
    $sugar_intake = $_POST['sugar-intake'];
    ...
    // Stress scoring
    if ($sugar_intake === 'Often') {
      $score += 2; // Add 2 if stress is Often
    } elseif ($sugar_intake === 'Sometime') {
    ...
    // Diet scoring
    if ($diet === 'Not Balanced') {
      $score += 2; // Add 2 if diet is Often
    } elseif ($diet === 'Somewhat Balanced') {
    ...
    if (isset($_SESSION['user_id'])) {
      ...
      $query = mysqli_query($con, "Insert into diabetes_info (user_id, family_history, sugar_intake, exercise, diet, risk_level) VALUES ('$user_id', '$history', '$sugar_intake', '$exercise', '$diet', '$risk_level')");
